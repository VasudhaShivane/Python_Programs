...
def setup_game():
    ...
    screen.bgpic('img5.gif')  # Set a background image (replace 'background.gif' with your image file)
    screen.setup(width=1000, height=600)

    l_paddle = turtle.Turtle()
    l_paddle.speed(0)
    #l_paddle.shape('left_paddle.gif')  # Replace 'left_paddle.gif' with your left paddle image file
    l_paddle.penup()
    l_paddle.goto(-400, 0)

    r_paddle = turtle.Turtle()
    r_paddle.speed(0)
    #r_paddle.shape('right_paddle.gif')  # Replace 'right_paddle.gif' with your right paddle image file
    r_paddle.penup()
    r_paddle.goto(400, 0)

    ball = turtle.Turtle()
    ball.speed(40)
    #ball.shape('ball.gif')  # Replace 'ball.gif' with your ball image file
    ball.penup()
    ball.goto(0, 0)
    ball.dx = 5
    ball.dy = -5
    ...
    score_board.speed(0)
    score_board.color('white')  # Set score color to white
    score_board.penup()
   ...
   while True:
       ...
       if ball.xcor() > 500:
           ...
           if l_score >= winning_score:
               score_board.clear()
               score_board.write("Left Player Wins!", align="center", font=("Arial", 24, "normal"))
               screen.update()
               screen.onkeypress(exit_game, 'Escape')
               screen.listen()
               screen.mainloop()
           ...
           if r_score >= winning_score:
               score_board.clear()
               score_board.write("Right Player Wins!", align="center", font=("Arial", 24, "normal"))
               screen.update()
               screen.onkeypress(exit_game, 'Escape')
               screen.listen()
               screen.mainloop()
